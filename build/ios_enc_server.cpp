...
vector<int>  test_vector_1;
vector<int> test_vector_2;
...
void read_vector() {
  ...
  for (int j = 0; j < 512; ++j) {
    ...
    int tt=(int)t;
    if (t > 0 && t - tt > 0.5) {
      test_vector_1.push_back( tt + 1);
    }
    else if (t < 0 && tt - t>0.5) {
      test_vector_1.push_back( tt - 1);
    }
    else {
      test_vector_1.push_back( tt);
    }
  }
  ...
  for (int j = 0; j < 512; ++j) {
    ...
    int tt=(int)t;
    if (t > 0 && t - tt > 0.5) {
      test_vector_2.push_back( tt + 1);
    }
    else if (t < 0 && tt - t>0.5) {
      test_vector_2.push_back( tt - 1);
    }
    else {
      test_vector_2.push_back( tt);
    }
  }
  infile.close();
...
/*
  ...
  */
  ...
  vector<int64_t> input1(n1,0);
  input1[0] = test_vector_1[0];
  for (int i = 1; i < l1; ++i){
    input1[n1-i] = -1 * test_vector_1[i];
    //input[n1-i] = 2;
  }
  vector<vector<int64_t> > ct1=RLWE64_Enc(input1,pk,p1,q1); 

  vector<int64_t> input2(n1,0);
  input2[0] = test_vector_2[0];
  for (int i = 1; i < l1; ++i){
    input2[n1-i] = -1 * test_vector_2[i];
    //input[n1-i] = 2;
  }
  vector<vector<int64_t> > ct2=RLWE64_Enc(input2,pk,p1,q1);      
  //vector<vector<int64_t> > ct=RLWE64_Enc_sk(input,x,p1,q1);     
  

  ofstream fout;
  fout.open("test_enc_nn.txt");
  ...
  fout.close();
  cout<<"Encrypt the plain network and store the encrypted network. "<<endl;

/*
  //for test
  vector<int64_t> pt1 = RLWE64_Dec(ct1, x, p1, q1);
  for(int i = 0 ; i < n1 ; ++i){
    cout <<"( "<<input1[i]<<", "<<pt1[i]<<") ";
  }
  vector<int64_t> pt2 = RLWE64_Dec(ct2, x, p1, q1);
  for(int i = 0 ; i < n1 ; ++i){
    cout <<"( "<<input2[i]<<", "<<pt2[i]<<") ";
  }
*/
